...
def preprocessing(case=1, strip_zeros=False):
    ...
    if case == 0:
        ...
        daily_data.drop(['Hash', 'PHash', 'PK'], axis=1)
...
def mlp(case, customer, postcode):
    ...
    if customer:
        ...
        print("NN number daily accuracy: ", accuracy_score(Y_test_num, mlp_predictions_num))
...
if __name__ == '__main__':
    ...
    if int(sys.argv[3]):
        print("Classifying stage 1 daily data with KNN")
        knn(case, True, True)

    if int(sys.argv[4]):
        print("Clustering stage 1 daily data with KMeans")
        kms(case)
