...
Use: python ./stage1_classify_daily.py [case] [MLP] [LSTM]
...
def mlp():
    ...
    print("NN number daily accuracy: ", accuracy_score(Y_test_num, mlp_predictions_num, normalize=True))
    ...
    print("NN postcode daily accuracy: ", accuracy_score(Y_test_post, mlp_predictions_post, normalize=True))
...
def lstm():
    ...
    print("NN number daily accuracy: ", accuracy_score(Y_test_num, lstm_predictions_num, normalize=True))

    print("Applying MLP neural network for postcode")
    #print("NN postcode daily accuracy: ", accuracy_score(Y_test_post, lstm_predictions_post, normalize=True))


if __name__ == '__main__':
    # Check usage
    if not len(sys.argv) == 4:
        print("Invalid usage: python ./stage1_classify_daily.py [case] [MLP] [LSTM]")
    ...
    if int(sys.argv[2]):
        print("Classifying stage 1 daily data with MLP neural network")
        mlp()

    if int(sys.argv[3]):
        print("Classifying stage 1 daily data with LSTM")
        lstm()
